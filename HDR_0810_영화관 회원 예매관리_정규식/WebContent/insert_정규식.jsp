...
function check() {
		...
		*/
		else if(!check2(regIDPass,me_id,"아이디를 4~12자리의 대소문자와 숫자로만 입력가능합니다"){ 
		...
		}else if(!check2(regIDPass,me_pass,"비밀번호를 4~12자리의 대소문자와 숫자로만 입력가능합니다"){ 
			//alert("비밀번호를 4~12자리의 대소문자와 숫자로만 입력가능합니다");
			return false;
		}
		...
		}else if(!check2(regIDPass,me_name,"비밀번호를 4~12자리의 대소문자와 숫자로만 입력가능합니다"){ 
			//alert("비밀번호를 4~12자리의 대소문자와 숫자로만 입력가능합니다");
			return false;
		}
...
function check2(re,id,message) {//정규식 = 정규화데이터, id , 메세지 
	if(re.test(id,value)){ //test() 입력한 값(id,value)이 정규화 데이터(re)를 사용했다
	return true;
	
	}
	alter(message); //메세지를 띄우고
	//방법-1
	//id.value ="";
	//return id.focus();
	//방법-2
	//return id.select();//아이디 있는부분에 블록을 하겠다
	//방법-3
	id.select();
	return false;
...
<body>
	...
	<tr>
		<th>회원아이디</th>
		<td><input type="text" name="me_id" id = "me_id" value="" size="10" maxlength="12">
			&nbsp; 4 ~ 12자의 영문 대소문자로 숫자로만 입력</td>
	</tr>

	<tr>
		<th>회원비밀번호</th>
		<td><input type="password" name="me_pass" id = "me_pass" value="" size="11" maxlength="15">
			&nbsp; 4 ~ 12자의 영문 대소문자로 숫자로만 입력</td>
	</tr>

	<tr>
		<th>회원이름</th>
		<td><input type="text" name="me_name"id="me_name" value="" size="10" maxlength="20"></td>
	</tr>
	...
	<tr>
		<th>전화번호</th>
		<td><input type="text" name="tel" value="" id = "tel"  size="20"maxlength="13">(예)010-1234-5678</td>
	</tr>
